...
export const BookVenueForm = ({
  ...
  const bookedDatesArray = Array.from(bookedDates).map((dateStr) =>
  ...
  async function onBookingSubmit(event) {
    ...
    if (!loggedInUser) {
      setIsError(
        <div className="not-logged-in-booking">
          <p>You need to be logged in to make a booking.</p>
          <span className="error-booking-div">
  ...
  async function updateBooking() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setIsError(
          "An error occured while trying to update your booking. Please try again",
        );
      }
    } catch (error) {
      setIsSuccess(false);
          ...
          <div className="modal-form-container">
            ...
            {isSuccess && (
              <div className="success-message">
                ...
                <Link className="go-back-link" onClick={closeModal}>
                  Go Back
                </Link>
              </div>
            )}
            {updateSuccess && (
              <div className="update-success-message">
                ...
                <Link className="go-back-link" onClick={closeUpdateBookingForm}>
                  Go Back
                </Link>
              </div>
            )}
